...
- Keep app-wide `next/font` setup in `src/app/_lib/fonts.ts` when root special files such as `not-found.tsx` or `global-error.tsx` need the same font variables. Export font variables/classes only; do not hide unrelated HTML/Tailwind classes in font helpers.
